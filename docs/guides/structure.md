...
> **FOR AGENTS:** При создании или обновлении документации строго следуй структуре соответствующего типа файла. Не добавляй лишних секций, не пропускай обязательные.

---

## Тип 1: Модель — `docs/models/<kebab-case>.md`

Описывает доменную модель: поля, связи, DTO-модели для CRUD.
...
- Если метод не использует сервисный слой (health) — не создавать service.md для этого модуля

---

## Тип 4: Внешний контракт — `docs/modules/<feature>/api.md`

Описывает queue контракты: команды, результаты, поведение, исключения.
...
- **Overview** — всегда 4 строки; если нет service (health) — пропустить Service file
- **Queues Summary** — таблица всех очередей модуля
- Для каждой очереди:
  - **Command** — все поля; `access_data` можно без Description
  - **Result** — поля результата; вложенные типы — inline списком: `**<NestedType>:** field1 (Type), field2 (Type)`
  - **Behavior** — только нетривиальная логика (создание сущностей, статус-коды, спецправила)
  - **Exceptions** — все возможные исключения с условиями
- Разделитель `---` между очередями
- Для пустых команд: `Пустая команда (без полей).`
- Если Result простой — можно без таблицы, только описание

### Правила для Result-схем

- **Переиспользуемые схемы** (используются в >1 очереди или >1 модуле) — выносятся в `docs/_shared/<schema-name>.md`. В очереди даётся ссылка: `→ См. [_shared/<file>.md](../../_shared/<file>.md)`.
- **Уникальные схемы** (используются только в одном модуле) — полная таблица в первом появлении. В остальных очередях модуля — ссылка на первое появление: `Та же схема, что и в [\`<queue>\`](#queue-<queue>).`

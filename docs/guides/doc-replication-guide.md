...
> **Note for agents:** Новые репозитории должны наследоваться от `BaseRepository[Model, CreateDTO, ReadDTO, UpdateDTO]` (или аналога). Описывайте только кастомные методы и eager loading флаги — базовые CRUD методы уже задокументированы в [base.md](repositories/base.md).

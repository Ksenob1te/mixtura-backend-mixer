...
| Repository | `src/infra/postgre/repo/<entity>.py` | — | `event.py` → `EventRepository` | `BaseRepository[...]` implementation |
...
class BaseRepository(Generic[ModelType, CreateDTO, ReadDTO, UpdateDTO]):
    ...
    async def update(self, dto: UpdateDTO) -> ReadDTO:
        ...
        ...  # fetch + setattr pattern (NOT merge)
    ...
    def _dto_to_data(self, dto: Any, *, exclude_unset: bool = False) -> dict[str, Any]:
        raw = dto.model_dump(exclude_unset=exclude_unset)
        return {k: v for k, v in raw.items() if k in self._column_names}
```
...
class BaseRepository(Generic[ModelType, CreateDTO, ReadDTO, UpdateDTO]):
    ...
    Subclasses set `model` and `dto_model` as class attributes and may
    override `get()` to support `load_*` flags with `selectinload()`.
    """
    ...
    def __init__(self, session: AsyncSession):
        ...
        if not hasattr(self.__class__, 'dto_model'):
        ...
        if hasattr(self.model, '__table__'):
            self._column_names = {c.name for c in self.model.__table__.columns}
    ...
    async def _flush(self) -> None:
        try:
            ...
            if pgcode_to_exception := {
                '23503': IntegrityForeignException,
                '23505': IntegrityUniqueException,
            }.get(pg_code):
                raise pgcode_to_exception() from exc
    ...
    def _dto_to_data(self, dto: Any, *, exclude_unset: bool = False) -> dict[str, Any]:
        if hasattr(dto, 'model_dump'):
            ...
            raw_data = dto if isinstance(dto, Mapping) else {}
        return {k: v for k, v in raw_data.items() if k in self._column_names}
    ...
    async def update(self, dto: UpdateDTO) -> ReadDTO:
        update_data = self._dto_to_data(dto, exclude_unset=True)
        pk = dto.id if hasattr(dto, 'id') else update_data.get('id')
        obj = await self._get_model(pk)
        if obj is None:
    ...
    async def exists(self, field_id: UUID) -> bool:
        stmt = select(func.count()).select_from(self.model).where(
            getattr(self.model, "id") == field_id,
        )

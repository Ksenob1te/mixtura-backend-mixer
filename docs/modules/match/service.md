...
### Clients
- `RatingClientProtocol` — `process_match_result()` (RabbitMQ publish, fire-and-forget). `calculate_effective_ratings()` реализован через неблокирующий `RabbitRpcClient` (aio_pika).

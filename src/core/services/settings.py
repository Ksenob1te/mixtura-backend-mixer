...
class SettingsService:
    ...
    async def add_integration(self, command: AddIntegrationCommand) -> EventDetail:
        ...
        if any(i.provider_id == command.provider_id for i in existing):
            raise ConflictException("Integration with this provider already exists")

        await self._integration_repo.create(
            RequiredIntegrationCreate(provider_id=command.provider_id, event_id=command.event_id)
        )
    ...
    async def get_application_form_settings(self, command: GetApplicationFormSettingsCommand) -> ApplicationFormSettingsResponse:
        ...
        return ApplicationFormSettingsResponse(
            ...
            required_integrations=[
                RequiredIntegrationResponse(id=i.id, provider_id=i.provider_id)

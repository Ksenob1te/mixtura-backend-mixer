...
class SwissSettingsRepository(BaseRepository):
    ...
    async def create(self, settings: SwissSettings) -> SwissSettings:
        self._session.add(settings)
        await self._flush()
        await self._session.refresh(settings)
        return settings

    async def update(self, settings: SwissSettings) -> SwissSettings:
